...
class PlayerTurn
{
    constructor(world, events, player_actor, possible_actions){
        ...
        this.player_actor = player_actor;           // Actor that represent the player for this turn.
...
// - A "Turn Phase" is when we go through all bodie's actors once, and if they can act (if they have enough action points),
...
// produced through the turns/actions of the different actors.
function* execute_turns_until_players_turn(world) {
    ...
    function* request_player_action(actor, possible_actions) { // Give back control to the player, request them to set an action in the World object.
        yield new PlayerTurn(world, events_since_last_player_action, actor, possible_actions);
    ...
    while(true){ // This is a virtually infinite sequence of turns.
        ...
        // Make sure there are actors ready to do act!
        if(world.actors.length == 0){ // No actors?
        ...
        let looping_actor_sequence = loop_actors_until_end_of_turn(world); // This sequence includes turn phases, so one character with enough action points left can occur more than one time.

        for(const actor of looping_actor_sequence){ // Actors can act several times a turn, but once a phase.
                                                  // There is a new turn phase if after cycling all actors, some still have action points left.
            console.assert(actor instanceof concept.Actor);
            /////////////////////////////////////////////////////////////
            // NEW CHARACTER TURN PHASE START HERE: CHOSE ONE ACTION!
            /////////////////////////////////////////////////////////////

            const possible_actions = world.gather_possible_actions_from_rules(actor);
            ...
            if(action == null){ // No decision taken? Only players can hesitate!!!!
                ...
                yield* request_player_action(actor, possible_actions); // Give back the control and the list of events done since last turn.
            ...
            const action_events = action.execute(world, actor);
            events_since_last_player_action.push(...action_events);

            // Update the world according to it's rules.
            const rules_events = world.apply_rules_end_of_characters_turn(actor);
...
// Generates a sequence of actors that can perform actions now.
// The filtered actors can appear only once each.
// This is basically a "Turn Phase".
// TODO: specify an order! maybe depending on stats?
function *actors_that_can_act_now(world){
    console.assert(world instanceof concept.World);
...
// Generates a sequence of actors until no actors can act for this turn.
function *loop_actors_until_end_of_turn(world){
    console.assert(world instanceof concept.World);
    let count_actors_that_acted = 0;
    do{
        count_actors_that_acted = 0;
        // New Turn Phase (if any actor can act)
        for(const actor of actors_that_can_act_now(world)){
            yield actor;
            ++count_actors_that_acted;
        }
    } while(count_actors_that_acted > 0);
    // No actors could act for this turn: END OF TURN!
}
